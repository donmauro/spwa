...
spa.chat = (function () {
  ...
  setPxSizes = function () {
    var px_per_em, windows_height_em, opened_height_em;

    px_per_em = spa.util_b.getEmSize( jqueryMap.$slider.get(0) );
    windows_height_em = Math.floor(
      ( jqueryMap.$window.height() / px_per_em ) + 0.5
    );
    opened_height_em
      = windows_height_em > configMap.windows_height_min_em
  ...
  // Purpose: Move the chat slider to the request position
  // Arguments: //  * position_type - enum('closed', 'opened', or 'hidden')
  // * callback - optional callback to be run at the end
  // of the slider animation. The callback receive a jQuery
  ...
  // return true if slider already in requested position_type
  ...
  writeChat = function ( person_name, text, is_user ) {
    var msg_class = is_user
      ? 'spa-chat-msg-log-me' : 'spa-chat_msg-log_msg';
  ...
  // Purpose:
  //   Given a windows resize event, adjust the presentation
  ...
  // Purpose : Configure the module prior the initialization
  // Arguments :
  // *  - a call back to modify the URI anchor
  //   indicate opened or closed state. This callback must return
  //   false if the request state cannot be method
  // * chat_model - the chat model object provides methods
  //   to interact with our instant messaging
  // * people_model - the people model object wich provides
  ...
  // Purpose    : Direct Chat to offer its capability to the user
  // Arguments  :
  ...
  //  it with HTML content. It then inizializes elements,
  //  events, and handlers to provided the user with a chat-room
  //  interface
  ...
  initModule = function ( $append_target ) {
    ...
    // initialize chat slider to default title and stateMap
  ...
  // Purpose:
  //  *  Remove chatSlider DOM elements
  //  *  Reverts to initial state
  //  *  Removes pointers to callback and other data
  // Arguments: none
  // Returns: true
  // Throws: none
